module Metadata
  class Authors
    ...
    def names_with_author_roles
      mods_ng_document.root&.xpath('mods:name[mods:role/mods:roleTerm[contains(text(), "AUT") ' \
      'or contains(text(), "aut") or contains(text(), "author") or contains(text(), "Author")]]', mods: MODS_NS)
    end

    def names_without_roles
      mods_ng_document.root&.xpath('mods:name[count(mods:role) = 0]', mods: MODS_NS)
    end

    def first_name_with_any_role
      mods_ng_document.root&.at_xpath('mods:name[mods:role]', mods: MODS_NS)
    end
  end
end
