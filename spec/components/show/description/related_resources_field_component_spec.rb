...
RSpec.describe Show::Description::RelatedResourcesFieldComponent, type: :component do
  subject(:component) { described_class.new(values: values) }
  ...
  context 'with a parker item with complex citation related resources' do
    ...
    it 'renders links that were re-categorized using displayLabel' do
      ...
      expect(rendered_content).to have_css('th', text: 'Superseded Interim Catalogue Record')
      expect(rendered_content).to have_link(href: 'https://stacks.stanford.edu/file/druid:gt628gx0397/2II.pdf')
    end
