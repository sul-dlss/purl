...
RSpec.describe 'Displaying the PURL page' do
  context 'with a map' do
    ...
    it 'displays the page' do
      visit "/#{druid}"
      expect(page).to have_text '[Plates to Allgemeine auf Geschichte und Erfahrung gegründete theoretisch-practische Wasserbaukunst.]'
  ...
  context 'with a file type' do
    ...
    it 'displays the page' do
      visit "/#{druid}"
      expect(page).to have_text 'Minutes, 2006 May 18'
      expect(page).to have_metadata_section 'Access conditions'
      expect(page).to have_metadata_section 'Description'
      expect(page).to have_metadata_section 'Contributors'
      ...
      expect(page).to have_text 'Stanford University, Cabinet, Records'
      expect(page).to have_text 'Stanford University. Cabinet, Stanford University--Administration'
  ...
  context 'with an image that is released to searchworks' do
    ...
    it 'displays the page' do
      visit "/#{druid}"
      expect(page).to have_text 'Atari Competition'
      expect(page).to have_metadata_section 'Access conditions'
      expect(page).to have_metadata_section 'Description'
      expect(page).to have_metadata_section 'Contributors'
      expect(page).to have_metadata_section 'Subjects'
      ...
      expect(page).to have_text 'Bay Area video arcades : photographs by Ira Nowinski, 1981-1982'
  ...
  context 'with an book that is part of collection not released to searchworks' do
    ...
    it 'lists the collection name and does not link to items in collection' do
      visit "/#{druid}"
      expect(page).to have_text 'Edward Flanders Ricketts papers, 1936-1979 (inclusive), 1936-1947 (bulk)'
  ...
  context 'with an invalid druid' do
    ...
    it '404 with invalid error message' do
      ...
      expect(page).to have_text 'The item you requested does not exist.'
    end
  end
  ...
  context 'with an incomplete/unpublished object (not in stacks)' do
    ...
    it 'gives unavailable message and a feedback form', :js do
      visit "/#{druid}"
      expect(page).to have_text 'The item you requested is not available.'
      expect(page).to have_text 'This item is in processing or does not exist. If you believe you have reached this page in error, please send Feedback.'
  ...
  context 'with more than one version' do
    ...
    it 'shows the version information panel' do
      visit "/#{druid}"
      expect(page).to have_text 'You are viewing this version'
      expect(page).to have_text 'Each version has a distinct URL, but you can use this PURL to access the latest version.'
    end
  end
